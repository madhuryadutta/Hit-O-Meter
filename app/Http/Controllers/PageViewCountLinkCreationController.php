...
class PageViewCountLinkCreationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($auth_status == 1) {
            $user_obj = Auth::user();
            $user_details = ['name' => $user_obj->name, 'email' => $user_obj->email];
            $current_user_id = Auth::id();
        } else {
            $current_user_id = 1;
            $user_details = ['name' => 'Annonymous', 'email' => 'Annonymous@who-m-i.com'];
        }
        ...
        try {
            ...
            $data1 = ['tracker_no' => $generate_tracker_id, 'host_type' => $validation_host_type, 'seurity_key' => $generate_security_code, 'friendly_name' => $validation_friendly_name, 'remark' => $validation_remark, 'creator' => $user_details['name'], 'creator_mail' => $user_details['email']];
            $data = compact('data1');
